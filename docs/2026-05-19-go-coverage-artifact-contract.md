...
CRAP uses the per-function coverage lines from the same command output. When
`gocyclo` reports a named function literal that `go tool cover -func` omits,
CRAP may fall back to raw profile blocks contained in the literal's exact source
